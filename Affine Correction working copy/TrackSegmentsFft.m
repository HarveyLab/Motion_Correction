function TrackSegmentsFft(fullfilename,movie_file,maxShift,nSegments)
...
% First, slice mov for parfor loop:
movSliced = cell(nSeg, 1);
for Seg = 1:nSeg
    movSliced{Seg} = mov(segPos(Seg,2):segPos(Seg,2)+segPos(Seg,4),segPos(Seg,1):segPos(Seg,1)+segPos(Seg,3),:);
end
...
for Seg = 1:nSeg
    ...
    [xshifts(Seg,:),yshifts(Seg,:)]=track_subpixel_wholeframe_motion_fft(...
