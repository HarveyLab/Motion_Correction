function [xShift, yShift] = ...
    track_subpixel_wholeframe_motion_fft(...
...
% For development: Load example segment:
% load('example60by60segment.mat');
% movref = stack;
% refframe = median(movref, 3);
% usfac = 30;
...
[rowShift, colShift] = dftregistration(mov_fft, ref_ftt, ref_fftshift, ref_conj, upsamplingFac);   
...
function [rowShift, colShift] = dftregistration(mov_fft, ref_ftt, ref_fftshift, ref_conj, usfac)
...
% Is this upsampling simply to accomodate the movement? Could we limit it
% to the expected maximum shift?
[m,n,z]=size(mov_fft);
...
CC(m+1-fix(m/2):m+1+fix((m-1)/2),n+1-fix(n/2):n+1+fix((n-1)/2), :) = ...
    bsxfun(@times, conj(fftshift(fftshift(mov_fft, 1), 2)), ref_fftshift);
...
upsampled = dftups(bsxfun(@times, mov_fft, ref_conj), nRow, nCol, usfac, offRow, offCol);
...
rg00 = dftups(ref_ftt.*ref_conj, 1, 1, usfac, 0, 0)/(md2*nd2*usfac^2);
rf00 = dftups(mov_fft.*conj(mov_fft), 1, 1, usfac, zeros(z, 1), zeros(z, 1))/(md2*nd2*usfac^2);  
...
for i = 1:nFrame
    ...
    kernc = exp((-1i*2*pi/(nc*usfac))*( ifftshift((0:nc-1)).' - floor(nc/2) )*( (0:nCol-1) - offCol(i) ));
    kernr = exp((-1i*2*pi/(nr*usfac))*( (0:nRow-1).' - offRow(i) )*( ifftshift((0:nr-1)) - floor(nr/2)  ));
