...
%     isAnatomicalZStack = nSlices>1 && ~siStruct.fastZEnable;
    ...
    if nFramesPerFile < nChannels*nFramesAvg*nSlices
        mov = reshape(mov, height, width, nChannels, min(nFramesAvg, nFramesPerFile), []);
        nSlices = size(mov, 5);
    else
        mov = reshape(mov, height, width, nChannels, min(nFramesAvg, nFramesPerFile), nSlices, []);
    end
    ...
    if isAnatomicalZStack
        ...
        for sl = 1:nSlices-(fZ*siStruct.fastZDiscardFlybackFrames)
            for ch = 1:nChannels            
